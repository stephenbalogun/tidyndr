---
title: "Extending the functionality of tidyndr"
...
1. `TX NEW NEW` 
2. `Continuity of Treatment Proxy`
...
### TX NET NEW
This indicator measures the increase (or decrease) in the number of clients on ART over a period of time. It is calculated by substracting the number of active clients in a previous period from the number of active clients from the current period.  
...
3. Summarise our line-list at the desired level ("ip", "state" or "facility") using the `summarise_ndr()`.  
4. Substract the value of the previous tx_curr from the current tx_curr.  
