...
The [Nigeria National Data Repository (NDR)](http://ndr.shieldnigeriaproject.com/) houses the de-identified patient-level information for the HIV program in Nigeria. Perhaps the most versatile feature of the repository is that it allows users with login access to download this patient-level information, analyze, and make informed decisions to improve the response of their program towards achieving the [UNAIDS](https://www.unaids.org/en) **three 95s** targets. 
...
The aim of the {tidyndr} package is to eliminate these hurdles by providing the user with an application that can be conducted on a free and open-source software, allows the user to focus on the task to be performed and not the formular, remove user-defined errors, and allow for easy automation of routine activities.  
...
4.	Adds to new columns to your data - `date_lost` (calculated by adding  28days to the sum of `last_drug_pick_up_date` and the `days_of_arv_refill`), and `current_status` (calculated by classifying the patient as “active” or “inactive” using the value of the `time_stamp` argument as a reference.   
...
* `quiet` - when this is set to FALSE (the default) it prints the names of the two columns added. 
* `...` - these are other arguments that can be passed to `vroom::vroom()`.
...
3. `to` - the end date for generating the requested line-list. This defaults to all             clients who started ART after the `from` date.
4. `states` and `facilities` - the particular state(s) of interest. When this is not supplied,it calculates the new clients for all states and facilities contained in the data.
...
* `status` - the column to be used to determine the tx_curr. Can be one of two - 'calculated' or 'default'. The 'calculated' (default) used the derived `current_status` column while the 'default' uses the NDR `current_status_28_days` column. 
...
### `tx_appointment `
Sometimes, you are interested in knowing the number of **Active** clients who are due for medication refill/drug pick up within a period of thime. This can help you to plan for the visits, forcast medications and also identify active clients who have missed their appointment. The `tx_appointment()` is one of the supporting treatment indicators that helps in this regard. It takes 6 arguments viz: 
...
## print the number of patients with appointments in Q2
nrow(q2_appt)
```
...
The viral load group of indicators allows you to be able to generate the list of clients who are eligible for viral load; have a viral load result within the last one year (for adults of 20 years and above) or 6 months (for paediatrics and adolescents); and are virally suppressed. These indicators include:    
...
### `tx_vl_eligible`
Two groups of clients are eligible for routine viral load test - clients who have been on ARV medication for at least 6 months and have not had a viral load result, and clients whose last viral load result is more than 12 months ago (for adults) and more than 6 months ago (for paediatrics and adolescents). `tx_vl_eligible()` generates this line-list using a combination of the 6 arguments below that can be provided:  
...
### `tx_pvls_num`
This determines the number of clients whose viral load test results are below a particular limit among those with a documented viral load result, thus are sadi to be virally suppressed. It helps you to know how many patients are doing well on their current medication. It accepts the following arguments:   
...
* `data`, `ref`, `states`, `facilities` and `status` - kindly see the documentation for [`tx_vl_eligible()`](#tx_vl_eligible) above.  
* `n` - the reference at which viral load values equal to or abov are said to be unsuppressed. The default is 1000.
...
### `disaggregate`
A very powerful function that allows you to summarise your generated line-list disaggregated based on a particular variable. The disaggregation options currently available are "age (or current age)", "sex (or gender)", "pregnancy status" or "population (Key Population or General Population)". It accepts 3 arguments:  
* `data` - see [`tx_new()`](#tx_new) documentation above.  
* `by` and `level` - see the documentation for [`summarise_ndr()`](#summarise_ndr) above.   

```{r, disaggregate}
## generate list of inactive clients
inactives <- tx_ml(ndr_example)
